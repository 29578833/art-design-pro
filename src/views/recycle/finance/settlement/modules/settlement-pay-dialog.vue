    ...
    <template v-if="row">
      <div class="fs-dialog-summary fs-dialog-summary--flat">
        ...
        <div class="fs-dialog-summary-line">
          ...
          <span
            >结算金额：<b class="fs-amount"
              >¥ {{ Number(row.final_price || 0).toLocaleString() }}</b
            ></span
          >
        </div>
      </div>
...
<script setup lang="ts">
  ...
  import { fetchSettlementBillPay } from '@/api/recycle/finance-settlement'
  ...
  async function handleSubmit() {
    if (!props.row || !selectedFile.value) {
      ElMessage.warning('请上传支付凭证')
      return
    }
    submitting.value = true
    try {
      const proof_image = await uploadFileGetUrl(selectedFile.value)
      await fetchSettlementBillPay(props.row.id, proof_image)
